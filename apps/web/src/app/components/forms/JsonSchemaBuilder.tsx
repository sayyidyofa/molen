...
import { FieldRow } from "./JsonSchemaBuilder/FieldRow";
...
export function JsonSchemaBuilder({ value, onChange }: JsonSchemaBuilderProps) {
  ...
  const addField = (parentPath: string[] = []) => {
    const newField: SchemaField = { id: `field-${Date.now()}`, name: "", type: "string", required: false };
    ...
    const updateRecursive = (fields: SchemaField[]): boolean => {
      for (let i = 0; i < fields.length; i++) {
        if (fields[i].id === id) {
          ...
          if (updates.type === "object" && !fields[i].properties) fields[i].properties = [];
          if (updates.type === "array" && !fields[i].items) {
            fields[i].items = { id: `${id}-items`, name: "items", type: "string", required: false };
          }
          return true;
        }
